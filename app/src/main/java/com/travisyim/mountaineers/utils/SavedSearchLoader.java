...
public class SavedSearchLoader {
    ...
    public static List<SavedSearch> load(List<ParseObject> resultList){
        ...
        for (ParseObject result : resultList) {
            ...
            savedSearch.setActivityStartDate(result.getDate(ParseConstants.KEY_ACTIVITY_START_DATE));
            savedSearch.setActivityEndDate(result.getDate(ParseConstants.KEY_ACTIVITY_END_DATE));

            try {
                savedSearch.setQueryString(result.getJSONArray(ParseConstants.KEY_KEYWORDS).join(" ").replaceAll("\"", ""));
            }
            ...
            savedSearch.setTypeUrbanAdventures(result.getBoolean(ParseConstants.KEY_TYPE_URBAN_ADVENTURE));
